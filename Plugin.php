...
class AMP_Plugin implements Typecho_Plugin_Interface
{
    ...
    public static function config(Typecho_Widget_Helper_Form $form)
    {
        ...
        $element = new Typecho_Widget_Helper_Form_Element_Text('baiduAPI', null, '', _t('百度MIP/AMP推送接口调用地址'), '<a href="https://ziyuan.baidu.com/mip/index">打开页面后 点击 百度MIP -> 数据提交 -> 提交新数据 获取接口调用地址。</a>（填写AMP或MIP的任意一个的提交地址即可）');
        $form->addInput($element);

        $element = new Typecho_Widget_Helper_Form_Element_Text('SMAPI', null, '', _t('神马MIP推送接口调用地址'), '<a href="https://ziyuan.baidu.com/mip/index">打开页面后 点击 神马MIP -> 数据提交 -> 提交新数据 获取接口调用地址。</a>（填写AMP或MIP的任意一个的提交地址即可）');
        $form->addInput($element);
